...
select year from movie m inner join rating r on m.mid=r.mid where r.stars > 3 order by(m.year) desc;
...
select name from reviewer re inner join rating ra on re.rid=ra.rid where ratingDate is null;
...
/*5.Write a query to return the ratings data in a more readable format: reviewer name, movie title, stars, and ratingDate. Also, sort the data, first by reviewer name, then by movie title, and lastly by number of stars. (1 point possible)*/
 ...
 select re.name,m.title,ra.stars,ra.ratingdate from movie m join rating  ra on m.mid=ra.mid join reviewer re on ra.rid=re.rid order by re.name,m.title,ra.stars;
...
/*7.For each movie that has at least one rating, find the highest number of stars that movie received. Return the movie title and number of stars. Sort by movie title. (1 point possible)*/
 ...
 select m.title,ra.stars from movie m join rating ra on m.mid=ra.mid group by(m.title) order by m.title;
...
select m.title,max(ra.stars)-min(ra.stars) as "rating_spread" from movie m join rating ra on m.mid=ra.mid group by(m.title) order by(rating_spread) desc,m.title;
...
/* 10.Find the names of all reviewers who rated Gone with the Wind. (1 point possible)*/
 ...
 select distinct(re.name) from reviewer re join rating ra on re.rid=ra.rid join movie m on m.mid=ra.mid where m.title like 'Gone with the Wind';
...
select title from movie order by name,title;


/* 13.Find the titles of all movies not reviewed by Chris Jackson. (1 point possible)*/

select distinct(title) from movie m inner join rating ra on ra.mid=m.mid join reviewer re on re.rid=ra.rid where re.name not like 'Chris Jackson';
